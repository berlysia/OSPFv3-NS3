...
Ptr<Ipv6Route> Ipv6OspfRouting::RouteOutput (
    ...
    if (destination.IsLinkLocalMulticast() && oif) {
        ...
        NS_LOG_INFO ("RouteOutput ()::LinkLocalMulticast destination - " << *rtentry);
        return rtentry;
    }
    ...
    if (destination.IsLinkLocal()) {
        ...
        if (oif) {
            rtentry->SetSource(m_ipv6->SourceAddressSelection (m_ipv6->GetInterfaceForDevice (oif), destination));
            rtentry->SetOutputDevice(oif);
            NS_LOG_INFO ("RouteOutput ()::LinkLocal destination - " << *rtentry);
            return rtentry;
        } else {
            for (InterfaceData& ifaceData : m_interfaces) {
                for (auto& kv : ifaceData.GetNeighbors()) {
                    // NS_LOG_INFO("router: " << m_routerId << ", iface: " << ifaceData.GetInterfaceId() << ", ifaceAddr: " << ifaceData.GetAddress() << ", neighAddr: " << kv.second.GetAddress());
                    // if (ifaceData.GetAddress() == destination) {
                    //     rtentry->SetSource(ifaceData.GetAddress());
                    //     rtentry->SetOutputDevice(0); // loopback

                    //     NS_LOG_INFO ("RouteOutput ()::LinkLocal destination - loopback! " << *rtentry);
                    //     return rtentry;
                    // }
            ...
            NS_LOG_INFO ("RouteOutput ()::LinkLocal destination - not found");
        }
    }
    ...
    if (destination.IsMulticast ())
    {
        ...
        NS_LOG_INFO ("RouteOutput ()::Multicast destination");
        // nop
    }
    ...
    if (rtentry)
    {
        ...
        NS_LOG_INFO ("RouteOutput ():: route found" << rtentry);
    }
    else
    {
        sockerr = Socket::ERROR_NOROUTETOHOST;
        NS_LOG_INFO ("RouteOutput ():: no route to host");
    }
    return rtentry;
}
...
void Ipv6OspfRouting::UpdateLSACaches(Ptr<OSPFLSA> lsa) {
    ...
    switch (lsa->GetHeader()->GetType()) {
        case OSPF_LSA_TYPE_LINK: {
            ...
            if (idx != -1 && idx < m_interfaces.size()) {
                m_interfaces[idx].AddLinkLocalLSA(advRtr, lsa);
                break;
            } else if (m_routerId == advRtr) {
                idx = m_ipv6->GetInterfaceForAddress(lsa->GetBody<OSPFLinkLSABody>()->GetLinkLocalAddress());
                if (idx != -1) {
                    m_interfaces[idx].AddLinkLocalLSA(advRtr, lsa);
                    break;
                }
...
uint16_t Ipv6OspfRouting::CalcMetricForInterface (uint32_t ifaceIdx) {
    ...
    if (netDevice->IsPointToPoint()) {
        ...
        auto val = dataRateValue.Get().GetBitRate();
        if (val <= 15260) {
            return 65535;
        }
...
void Ipv6OspfRouting::ReceiveLinkStateUpdatePacket(uint32_t ifaceIdx, Ptr<Packet> packet) {
    ...
    for (auto received : lsuPacket.GetLSAs()) {
        NS_LOG_INFO("iterate for: " << *received);
        ...
        if (hasInLSDB && !(isMoreRecent || isSameInstance)) {
            ...
            if (GetLastLSUSentTime() + g_minLsArrival <= ns3::Now()) {
                // LSUでDB内のLSAを直接送り返す
        ...
        if (neighData.HasInRxmtList(identifier)) {
            if (isSameInstance) {
                ...
                NS_LOG_INFO("RemoveFromAllRxmtList - router " << m_routerId << " の neighbor " << neighborRouterId << " から削除します: " << identifier);
                // NS_LOG_DEBUG("RemoveFromAllRxmtList - before " << neighData.GetRxmtList());
                neighData.RemoveFromRxmtList(identifier);
                // NS_LOG_DEBUG("RemoveFromAllRxmtList - after " << neighData.GetRxmtList());
...
void Ipv6OspfRouting::DirectAppendToRxmtList (Ptr<OSPFLSA> lsa, uint32_t ifaceIdx, RouterId neighborRouterId, bool sendAsap) {
    ...
    if (sendAsap) {
        ...
        AddToRxmtList(ifaceIdx, neighborRouterId, lsa);
    }
}
    ...
    for (auto ifaceIdx : m_rtrIfaceId_set) {
        ...
        for (auto& kv : ifaceData.GetNeighbors()) {
            ...
            if (sendAsap) {
                ...
                AddToRxmtList(ifaceIdx, kv.first, lsa);
            }
...
void Ipv6OspfRouting::RemoveFromAllRxmtList(OSPFLinkStateIdentifier &identifier) {
    ...
    for (InterfaceData& ifaceData : m_interfaces) {
        if (!ifaceData.IsActive()) continue;
        for (auto& kv : ifaceData.GetNeighbors()) {
            NS_LOG_INFO("RemoveFromAllRxmtList - router " << m_routerId << " の neighbor " << kv.first << " から削除します: " << identifier);
            // NS_LOG_DEBUG("RemoveFromAllRxmtList - before " << kv.second.GetRxmtList());
            kv.second.RemoveFromRxmtList(identifier);
            // NS_LOG_DEBUG("RemoveFromAllRxmtList - after " << kv.second.GetRxmtList());
        }
    }
}

void Ipv6OspfRouting::AddToRxmtList(int32_t ifaceIdx, Ptr<OSPFLSA> lsa) {
    if (ifaceIdx <= 0) return;
    InterfaceData& ifaceData = m_interfaces[ifaceIdx];
    for (auto& kv : ifaceData.GetNeighbors()) {
        NS_LOG_INFO("AddToRxmtList - router " << m_routerId << " の neighbor " << kv.first << " に追加します: " << lsa->GetIdentifier());
        // NS_LOG_DEBUG("AddToRxmtList - before " << kv.second.GetRxmtList());
        kv.second.AddRxmtList(lsa);
        // NS_LOG_DEBUG("AddToRxmtList - after " << kv.second.GetRxmtList());
    }
}

void Ipv6OspfRouting::AddToRxmtList(int32_t ifaceIdx, RouterId neighborRouterId, Ptr<OSPFLSA> lsa) {
    if (ifaceIdx <= 0) return;
    InterfaceData& ifaceData = m_interfaces[ifaceIdx];
    NeighborData& neighData = ifaceData.GetNeighbor(neighborRouterId);

    NS_LOG_INFO("AddToRxmtList - router " << m_routerId << " の neighbor " << neighborRouterId << " に追加します: " << lsa->GetIdentifier());
    // NS_LOG_DEBUG("AddToRxmtList - before " << neighData.GetRxmtList());
    neighData.AddRxmtList(lsa);
    // NS_LOG_DEBUG("AddToRxmtList - after " << neighData.GetRxmtList());
}
    ...
    case NeighborEvent::NEGOT_DONE: {
        if (neighbor.IsState(NeighborState::EXSTART)) {
            ...
            for(auto maxAgedLsa : rxmt) {
                AddToRxmtList(ifaceIdx, neighborRouterId, maxAgedLsa);
            }
            Simulator::ScheduleNow(&Ipv6OspfRouting::SendDatabaseDescriptionPacket, this, ifaceIdx, neighborRouterId, false);
        }
        break;
...
void Ipv6OspfRouting::SendHelloPacket(uint32_t ifaceIdx) {
    ...
    // hello.Print(std::clog);

    Ptr<Packet> packet = Create<Packet>();
    packet->AddHeader(hello);

    int n = m_ipv6->GetNAddresses(ifaceIdx);
    for (int i = 0; i < n; ++i) {
        NS_LOG_INFO("addr["<<i<<"]: " << m_ipv6->GetAddress(ifaceIdx, i));
    }
...
void Ipv6OspfRouting::SendLinkStateUpdatePacket(uint32_t ifaceIdx, RouterId neighborRouterId) {
    ...
    for (auto& item : tmp) {
        item->AgingBeforeFlooding(ifaceData.GetIfaceTransDelay());
    }
    lsu.SetLSAs(tmp);

    NS_LOG_INFO("Sending LSU("<<m_routerId<<", "<<neighborRouterId<<"): " << lsu);
...
Ptr<Ipv6Route> Ipv6OspfRouting::Lookup(Ipv6Address src, Ipv6Address dst, Ptr<NetDevice> interface) {
    ...
    bool isSender = src == Ipv6Address::GetZero() || m_ipv6->GetInterfaceForAddress(src) != -1;
    bool isReceiver = m_ipv6->GetInterfaceForAddress(dst) != -1;
    RouterId srcRtr = isSender ? m_routerId : m_routingTable.GetNearestRouter(src);
    ...
    if (isReceiver && srcRtr == m_routerId) {
        ...
        srcRtr = m_routingTable.GetNearestRouter(src, m_routerId);
    }

    if (!isSender && !isReceiver && dstRtr == m_routerId) {
        // dstでもないのに自分が目的地になってしまう場合、目的ルータはお隣のはず
        // 他のルータも自分を目的ルータだと思うはず -> つまり分散させるとループになる
        // これを知りうるのは自分だけなので、ここで直送することで回避する
        nextHopRouterId = m_routingTable.GetNearestRouter(dst, m_routerId);
        
        // 失敗する場合、OSPF管理下にない直結ノードが宛先になる
        if (nextHopRouterId == 0) {
            for (uint32_t ifaceIdx = 0, l = m_ipv6->GetNInterfaces(); ifaceIdx < l; ++ifaceIdx) {
                for (uint32_t addrIdx = 0, l = m_ipv6->GetNAddresses(ifaceIdx); addrIdx < l; ++addrIdx) {
                    auto ifaceAddr = m_ipv6->GetAddress(ifaceIdx, addrIdx);
                    if (
                        ifaceAddr.GetAddress () != Ipv6Address () &&
                        ifaceAddr.GetPrefix () != Ipv6Prefix () &&
                        ...
                        route->SetGateway(Ipv6Address::GetZero()); // nexthop
    ...
    if (nextHopRouterId == 0) {
        nextHopRouterId = m_routingTable.LookupRoute(srcRtr, dstRtr);
    }
    ...
    if (nextHopRouterId != 0) {
        ...
        route->SetDestination(dst); // 
        route->SetGateway(m_interfaces[ifaceIdx].GetNeighbor(nextHopRouterId).GetAddress()); // nexthop
        route->SetOutputDevice(m_ipv6->GetNetDevice(ifaceIdx));
        return route;
    }
...
typedef std::unordered_map<RouterId, uint16_t> NextHopEntries;
// typedef std::vector<std::vector<NextHopEntries> > RoutingTable;
...
void Ipv6OspfRouting::CalcRoutingTable (bool recalcAll) {
    ...
    std::vector<std::vector<NextHopEntries> > routingTable; // [self][from][to][next] =  flow;

    capacityTable.resize(routers, std::vector<uint16_t>(routers, 0));
    ...
    std::queue<RouterId> que;
    std::stack<RouterId> sta;
    std::vector<int32_t> level(routers);
    std::vector<uint32_t> iter(routers);
    RouterId curr, fr, to;
    uint32_t routeFlow = 65535;
    for (uint32_t src = 1, cl = routers; src < cl; ++src) {
        for (uint32_t dst = 1, rl = routers; dst < rl; ++dst) {
            if (src == dst) continue;
            ...
            for (;;) {
                ...
                while (!sta.empty()) {
                    ...
                    if (curr == dst) {
                        ...
                        RouterId nextHop = MAX_ROUTER_ID;
                        while (sta.size() >= 2) {
                            to = sta.top(); sta.pop();
                            fr = sta.top();
